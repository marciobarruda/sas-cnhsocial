...
# Use relative path to avoid "Automação" encoding issues
$payloadPath = Join-Path $PSScriptRoot "payload_utf8.json"
[System.IO.File]::WriteAllText($payloadPath, $json, [System.Text.Encoding]::UTF8)

try {
    # Using curl.exe with the payload file
    curl.exe -X PUT -H "X-Redmine-API-Key: $apiKey" -H "Content-Type: application/json; charset=utf-8" --data-binary "@$payloadPath" $uri
    Write-Host "`nUpdate command executed."
} catch {
